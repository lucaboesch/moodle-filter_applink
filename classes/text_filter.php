...
if (class_exists('\core_filters\text_filter')) {
    class_alias('\core_filters\text_filter', 'filter_applink_base_text_filter');
} else {
    class_alias('\moodle_text_filter', 'filter_applink_base_text_filter');
}

/**
 * Moodle App link content filter.
 *
 * Given some text, treat all the anchors that have the data-app-link attribute, converting them to a
 ...
 */
class text_filter extends \filter_applink_base_text_filter {
    ...
    protected function get_attr_data($name, $text) {
        ...
        if (!empty($matches)) {
            return [$matches[0], count($matches) > 2 ? $matches[2] : ''];
        }
    ...
    protected function is_ws_access() {
        ...
        if ($ME !== null && strpos($ME, 'webservice/') !== false) {
